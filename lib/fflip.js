 ...
 * @return {Promise} Returns true if ALL criteria are met, false otherwise.
 */
function evaluateCriteriaSet(criteriaSet, user) {
	var promises = Object.keys(criteriaSet).map(function(cName) {
		if (cName === '$veto') {
		...
		return Promise.resolve()
			.then(function() {
				return c_func(user, c_data);
			})
			.catch(function(err) {
				// If any error happened then return false
				return false;
			});
	});
	return Promise.all(promises).then(function(resultes) {
		return resultes.every(function(value) {
			return value === true;
		});
	});
}
 ...
 * @param {Object} user The expected user object to check against in each criteria function.
 * @return {Promise} Returns true if if ANY one member evaluates to true, and no "vetoing"
 * members evaluate to false. Returns false otherwise.
 */
function evaluateCriteriaList(criteriaList, user) {
	var promises = criteriaList.map(function(listMember) {
		return Promise.resolve()
			.then(function() {
				if (Array.isArray(listMember)) {
					...
					return evaluateCriteriaList(listMember, user);
				} else {
					// if object, evaluate all and return true if ALL return true
					return evaluateCriteriaSet(listMember, user);
				}
			})
	...
	return Promise.all(promises).then(function(resultes) {
		return resultes.some(function(value) {
			return value === true;
		});
	});
}
...
var self = module.exports = {
	 ...
	 * @return {Promise} Return true if user has this feature, false otherwise.
	 */
	 ...
	 * @return {Promise} The collection of all features and their availability.
	 */
	userFeatures: function(user, flags) {
		flags = flags || {};
		var userFeatures = {};

		var promises = Object.keys(self._features).map(function(featureName) {
			if (self._features.hasOwnProperty(featureName)) {
				if (flags[featureName] !== undefined) {
					...
					return Promise.resolve()
						.then(function() {
							return self.userHasFeature(user, featureName);
						})
						.then(function(enabled) {
							userFeatures[featureName] = enabled;
						});
				}
			} else {
				return null;
